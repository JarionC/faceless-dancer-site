...
export function saveGameControlDefaults(params: {
  ...
  ).run(analysisJson, laneJson);
}

export function getGameControlDefaults(): {
  analysisOverrides: Record<string, unknown> | null;
  laneStrengthThresholds: Record<string, unknown> | null;
} {
  const row = db
...
export function listAllSongs(): Array<GameSongRow> {
  return db
    .prepare(
      `SELECT id, beat_entry_id, title, is_enabled, cover_image_file_name, created_at, updated_at
       FROM game_songs
       ORDER BY updated_at DESC`
    )
    .all() as Array<GameSongRow>;
}

export function listEnabledSongs(): Array<{ beatEntryId: string; title: string; coverImageFileName: string | null }> {
  return db
    .prepare(
    ...
    .all() as Array<{ beatEntryId: string; title: string; coverImageFileName: string | null }>;
}

export function findSongByEntryId(beatEntryId: string): GameSongRow | undefined {
  return db
    .prepare(
      `SELECT id, beat_entry_id, title, is_enabled, cover_image_file_name, created_at, updated_at
       FROM game_songs
    ...
    .get(beatEntryId) as GameSongRow | undefined;
}

export function isEntryEnabled(beatEntryId: string): boolean {
  const row = db
    .prepare(`SELECT is_enabled FROM game_songs WHERE beat_entry_id = ? LIMIT 1`)
    .get(beatEntryId) as { is_enabled: number } | undefined;
  return row?.is_enabled === 1;
}

export function upsertSongForEntry(params: {
  beatEntryId: string;
  title: string;
  isEnabled: boolean;
  createdByUserId: string;
}): GameSongRow {
  const existing = findSongByEntryId(params.beatEntryId);
  if (!existing) {
    const id = createId();
    db.prepare(
      `INSERT INTO game_songs (id, beat_entry_id, title, is_enabled, created_by_user_id)
       VALUES (?, ?, ?, ?, ?)`
    ).run(id, params.beatEntryId, params.title, params.isEnabled ? 1 : 0, params.createdByUserId);
    return findSongByEntryId(params.beatEntryId)!;
  }

  db.prepare(
    `UPDATE game_songs
     SET title = ?, is_enabled = ?, updated_at = datetime('now')
     WHERE beat_entry_id = ?`
  ).run(params.title, params.isEnabled ? 1 : 0, params.beatEntryId);

  return findSongByEntryId(params.beatEntryId)!;
}

export function setSongCoverImageForEntry(beatEntryId: string, coverImageFileName: string | null): GameSongRow | undefined {
  db.prepare(
    `UPDATE game_songs
     SET cover_image_file_name = ?, updated_at = datetime('now')
     WHERE beat_entry_id = ?`
  ).run(coverImageFileName, beatEntryId);
...
export function createScore(params: {
  ...
  const song = findSongByEntryId(params.beatEntryId);
  ...
  return db
    .prepare(
      `SELECT
         ...
         MAX(gs.poor) AS poor,
         MAX(gs.miss) AS miss,
         MAX(gs.created_at) AS updatedAt
       FROM game_scores gs
...
export function listOverallLeaderboard() {
  return db
    .prepare(
    ...
    .all() as Array<{ displayName: string; publicKey: string; totalScore: number; songsCount: number }>;
}
