...
export function createPrecisePlaybackEngine(): PrecisePlaybackEngine {
  ...
  const getRenderedContextTime = (): number => {
    const maybeGetOutputTimestamp = (context as unknown as {
      getOutputTimestamp?: () => AudioTimestamp;
    }).getOutputTimestamp;
    const ts = maybeGetOutputTimestamp?.call(context);
    if (ts && Number.isFinite(ts.contextTime ?? Number.NaN)) {
      return ts.contextTime ?? 0;
    }
